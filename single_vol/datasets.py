    ...
    def __init__(
        ...
        for vol_id, file in enumerate(files):
            ...
            if with_mask:
                ...
                # kx_ids = torch.from_numpy(np.setdiff1d(np.arange(width), np.arange(left_idx, right_idx)))
                print('Fully sampled kspace training')
                kx_ids = torch.arange(width) # NOTE: Uncomment to include all the datapoints (fully-sampled volume), with the exception of the center region.
            ...
            quant_mod = torch.quantile(torch.view_as_complex(targets).abs(), 0.999)
