...
class HDRLoss:
    ...
    def __call__(self, predictions, targets, kcoords):
        ...
        loss = (error.abs()/(predictions.detach().abs() + self.epsilon))**2
        ...
        if self.factor > 0.0:
            ...
            reg = self.factor * (reg_error.abs() / (predictions.detach().abs() + self.epsilon)) ** 2
...
#         loss = ((predictions - targets).abs() / (predictions.detach().abs() + self.epsilon)) ** 2
